...
* Boot firmware
#+INCLUDE: "./boot_firmware.org" :minlevel 2
** COMMENT LINK: [[file:boot_firmware.org][Boot firmware]]

* Installing Linux (Ubuntu LTS 22.04 option)
...
~http://localhost:8888/tree?token~) to access the remote Notebook system.
...
* Installing Linux (PetaLinux option)                      :WORK_IN_PROGRESS:
This part is an alternative to the previous section [[]], where the installation
of PetaLinux as a host system[fn:24] will be presented, as being an alternative to the
...
# Jupyter can also be activated as a packagegroup ?!
...
#+ATTR_LATEX: :width .6\textwidth
#+CAPTION: Hey, listen !
#+NAME: fig:yocto_openamp2
[[file:./img/yocto_openamp2.png]]
...
The initial login username is ~petalinux~. The initial password is to be set at the moment of the first boot.
...
** Note: good points for PetaLinux over Ubuntu
...
+ +The fan is silent all the way through, and seemingly managed in a better way.+
  ...
  controll system, as visible in the ~dmesg~ log.
+ OpenAMP and the echo test app works fairly "out of the box"
+ Communication seems insanely fast too
...
** Building a eProsima bare-metal example agent
...
** WAIT Separated RPMsg transport created for XRCE-DDS Agent
...
[fn:29] The ~xilinx-kr260-starterkit-v2023.2-10140544.bsp~ file is specifically
...
[fn:26] https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-design-tools.html 
