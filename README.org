...
The default username / password pair for the very first boot is ~ubuntu~ and ~ubuntu~. You will then be prompted to enter a new passwork.
...
#+RESULTS:
| patching | file | echo_test.c |                    |                    |
| cc       | -c   | -o         | echo_test.o         | echo_test.c         |
| cc       | -c   | -o         | ../common/common.o | ../common/common.c |
| cc       | -o   | echo_test   | echo_test.o         | ../common/common.o |
|          |      |            |                    |                    |
| Echo     | test | start      |                    |                    |
...
* Footnotes
[fn:38] In this example, the full ~username@IP~ is used, but a ~.ssh/config~ is aslo usable. 

[fn:37] For the routing part, it is key to have the ~to~ with a ~'-'~ in front of
it; and then the ~via~ without, but alligned with the ~t~.
