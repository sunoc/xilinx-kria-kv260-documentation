...
RTOS. The figure \ref{fig:org9e8e352} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/map.png}
\caption{\label{fig:org9e8e352}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}

\section{Boot firmware}
\label{sec:org8f44f69}
...
figure \ref{fig:org3303f73} below.
...
the IPv4 as manual, as visible in the figure \ref{fig:orgaeae03c} hereafter.
...
shall now see the interface, as visible on the figure \ref{fig:org8ceb670} below. If
...
\caption{\label{fig:org3303f73}Port to be used for the Ethernet Boot Recovery Tool access on the KR260 board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/ipv4set.png}
\caption{\label{fig:orgaeae03c}IPv4 settings for accessing the Recovery Tool.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{img/recovery.png}
\caption{\label{fig:org8ceb670}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsubsection{Updating the boot firmware}
\label{sec:orgccacbfb}
...
(as of 2023-08-30).}. The figure \ref{fig:org498886d} below shows
...
\caption{\label{fig:org498886d}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:org258e8df}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:org96d2ec1} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:org96d2ec1}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\subsubsection{Enabling \texttt{remoteproc} with Device-Tree Overlay patching}
\label{sec:orge08bb4a}
...
but also in this report appendix \ref{sec:orgfe36181}.
...
in the figure \ref{fig:org9833e4c} below, after the commands:
...
\caption{\label{fig:org9833e4c}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsubsection{Adding a swap partition}
\label{sec:orgcdc720f}
...
\label{sec:orgdce7618}
\section{Setting up a Vitis IDE project}
\label{sec:org175d7ca}
...
The figure \ref{fig:orge19f9bc} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:orge19f9bc}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsubsection{Setting up the IDE}
\label{sec:org73ca16c}
...
\item Platform configuration file generation
\label{sec:org28a1129}
...
\caption{\label{fig:org34a3888}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:org1ee2d62}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:orgde1ae4a}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:orgc7abc7d}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org917dcb8}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:orgc8c8e22}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org455a413}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:orgc7ccdfa} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:orgc7ccdfa}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}
...
\label{sec:org5e3d35e}
\section{{\bfseries\sffamily TODO} R5F firmware without an IDE}
\label{sec:org597266a}
\pagebreak
\section{RPMsg \texttt{echo\_test} software}
\label{sec:orge21fc31}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:orgb1e3bb8}
and \ref{sec:org83d7d6c} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:org458444a} and \ref{fig:org8854051} below will show you a setup that worked to have the IDE
...
The end result for these implementation are visible in the appendix \ref{sec:org8b36a8e},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:orgb88471b}
...
is showed and detailed in the figure \ref{fig:org8ee1018} below.

The next figure \ref{fig:org09cc6cc} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:org8ee1018}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:org09cc6cc}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org73ca16c}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:org8ee1018}, here are the main steps
...
is visible in the section \ref{sec:orge21fc31}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
As for the previous section, the figure \ref{fig:org8dca4fd} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org8dca4fd}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:org2bdedf5}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:org5a7ceeb}
...
as presented in section \ref{sec:org4bbf9fc}.
...
and as presented in the section \ref{sec:org5a7ceeb} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
source /opt/ros/$ROS_DISTRO/setup.bash
...
on the schematic of the figure \ref{fig:org05c47c8} below.
...
\caption{\label{fig:org05c47c8}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{[irrelevent] Building a eProsima bare-metal example agent}
\label{sec:orgb1c6afd}
...
\label{sec:org8b36a8e}

\subsection{main}
\label{sec:org5abd1c3}
...
\label{sec:org5f78042}

\subsection{main}
\label{sec:orgc095028}
...
\label{sec:orgb02dd11}
This part is an alternative to the previous section \ref{sec:org4e42a54},
...
board's latest updated boot firmware, as presented in the Section \ref{sec:org8f44f69}
...
will also be using this version.}\footnotetext[43]{\label{org544c089}\url{https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria+SOMs+Starter+Kits\#K26-Boot-Firmware-Updates}}.
\end{itemize}
...
setting the the DTG Settings, as visible in the Figure \ref{fig:org5443844} below:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
\caption{\label{fig:org5443844}In the hardware config, it is possible to select the openamp device tree source.}
\end{figure}
...
\caption{\label{fig:orgfb9fd02}In the rootfs config, it is possible to select the openamp package.}
\end{figure}
...
\caption{\label{fig:org3ac0805}OpenSSH can be configure in the same rootfs config.}
\end{figure}
...
The figure \ref{fig:org96a2dd6} below shows a comparison for the data
...
\caption{\label{fig:org96a2dd6}A graph of the data transfer rate for individual packages to be transmitted and received back from the Linux.}
\end{figure}
