...
capable to run a RTOS. The figure \ref{fig:org146df22} below shows a schematic
view of the overall system we are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map.png}
\caption{\label{fig:org146df22}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}


\section{Boot firmware}
\label{sec:orgfab6d07}
...
the figure \ref{fig:org2ee8156} below. If the page struggle to appear,
\end{itemize}
...
\caption{\label{fig:org2ee8156}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsection{Updating the boot firmware}
\label{sec:org64b78ac}
...
(as of 2023-08-30).}. The figure \ref{fig:orgddc4f5c} below shows
...
\caption{\label{fig:orgddc4f5c}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:org49ce330}
...
but also in this report appendix \ref{sec:org234f48e}.
...
The figure \ref{fig:org65473fa} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:org65473fa}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org3fe65b0}
...
\caption{\label{fig:org4e9c65b}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.5\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:org5fb4cdc}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:orgbeea67c}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:org7098455}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org3a3b797}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:orgd30a63f}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org3dc96ad}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org5a4876f} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org5a4876f}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}


Once your example project is built and you have a \texttt{.elf} file available, you can
jump directly to the \ref{sec:orgf8d8b1c} section to see how to deploy and use your firmware.

The section in between will present setup specifically needed for micro-ROS.

\subsection{Enabling the Stream Buffer system}
\label{sec:orge1ea159}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:org088cdbd}
and \ref{sec:org726a5b3} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:org7d0754d} and \ref{fig:org5a104f4} below will show you a setup that worked to have the IDE
...
\caption{\label{fig:org7d0754d}Firstly, in the "C/C++ Build" settings of your firmware project, under the "Settings" menu, you should find the gcc compiler "Directories". In here you should add the "include" directory of your library. Be careful however, if your include files are in a second layer of directory (as it is the case for libmicroros) you will need to include each sub-directory individually, as visible in this figure.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{./img/vitis_new/include2.png}
\caption{\label{fig:org5a104f4}Secondly, in the gcc linkers "Libraries", you can add the top level directory of your library. In our case, it is the directory that contains both the "include" directory added earlier, and also the "libmicroros.a" file.}
\end{figure}
...
The end result for these implementation are visible in the appendix \ref{sec:org085ee0e},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:org1e8cf4d}
...
is showed and detailed in the figure \ref{fig:org931657f} below.
...
\caption{\label{fig:org931657f}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org3fe65b0}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:org931657f}, here are the main steps
...
is visible in the section \ref{sec:orgfbff660}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
As for the previous section, the figure \ref{fig:org91ceceb} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org91ceceb}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:org9e3ac89}
...
in the figure \ref{fig:org9b719a6} below, after the commands:
...
\caption{\label{fig:org9b719a6}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}


\subsubsection{Running a ROS2 container}
\label{sec:orgde89d69}
...
and as presented in the section \ref{sec:org9e3ac89} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
on the schematic of the figure \ref{fig:org9c32eb2} below.
...
\caption{\label{fig:org9c32eb2}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}


\subsection{Building a eProsima agent bare-metal}
\label{sec:org0768d21}
...
\begin{itemize}
\item The micro-ROS firmware is running on the R5F core, as presented in the section \ref{sec:orgf8d8b1c}.
\item On the same Linux you plan to run the node, you will need to run and keep the Agent, as presented
in the section \ref{sec:org94c6577}.
\item This ping-pong application is released as a ROS2 Python node. It is best to pull and run it
in a ROS2 environment to avoid having dependancies issues, as presented in the section \ref{sec:orgc8c91c3}.
\end{itemize}

\section{Conclusion \& future\hfill{}\textsc{WORK\_IN\_PROGRESS}}
\label{sec:org8101c8b}
...
\label{sec:org085ee0e}

\subsection{main}
\label{sec:orgb4ed2b0}
...
\label{sec:org56823ea}

\subsection{main}
\label{sec:orga966105}
