...
RTOS. The figure \ref{fig:org08d58ed} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map.png}
\caption{\label{fig:org08d58ed}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}

\section{Boot firmware}
\label{sec:orgb1c32f7}
...
\label{sec:orgb9d41ed}
A 2022 version of the board firmware is required in order to run the latest
version of Ubuntu properly.

The image can be downloaded at the atlassian page\footnote{\url{https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria}} on the topic,
...
the figure \ref{fig:org2e23da0} below. If the page struggle to appear,
\end{itemize}
...
\caption{\label{fig:org2e23da0}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsubsection{Updating the boot firmware}
\label{sec:org718af0b}
...
(as of 2023-08-30).}. The figure \ref{fig:org2d6cdd3} below shows
...
\caption{\label{fig:org2d6cdd3}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:orgffadc03}
...
\texttt{http://localhost:8888/tree?token}) to access the remote Notebook system.
It is possible to do so with \texttt{localhost} since we have the \texttt{ssh} port map connection going on.

Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:org234ee99} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jupyter.png}
\caption{\label{fig:org234ee99}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\section{Installing Linux (PetaLinux option)\hfill{}\textsc{WORK\_IN\_PROGRESS}}
\label{sec:org489ecec}
...
\caption{\label{fig:org2536566}Hey, listen \textasciitilde{}}
\end{figure}
...
\caption{\label{fig:orge051bbe}Hey, listen !!}
\end{figure}
...
\caption{\label{fig:orge3ba1e9}Hey, listen !}
\end{figure}
...
but also in this report appendix \ref{sec:org8a3acec}.
...
The figure \ref{fig:orgffddd16} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:orgffddd16}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org174a29b}
...
\caption{\label{fig:org9e64d5f}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:orge6b5647}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:org327755e}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:org90a4872}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org5a8be7b}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:org06ea380}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org8c80503}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org624c0d5} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org624c0d5}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}


Once your example project is built and you have a \texttt{.elf} file available, you can
jump directly to the \ref{sec:orgebffefa} section to see how to deploy and use your firmware.

The section in between will present setup specifically needed for micro-ROS.

\subsection{Enabling the Stream Buffer system}
\label{sec:org5dca4fc}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:org83b6ba8}
and \ref{sec:orge5d096c} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:org84bcbc5} and \ref{fig:orgc96a59d} below will show you a setup that worked to have the IDE
...
\caption{\label{fig:org84bcbc5}Firstly, in the "C/C++ Build" settings of your firmware project, under the "Settings" menu, you should find the gcc compiler "Directories". In here you should add the "include" directory of your library. Be careful however, if your include files are in a second layer of directory (as it is the case for libmicroros) you will need to include each sub-directory individually, as visible in this figure.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{./img/vitis_new/include2.png}
\caption{\label{fig:orgc96a59d}Secondly, in the gcc linker "Libraries", you can add the top level directory of your library. In our case, it is the directory that contains both the "include" directory added earlier, and also the "libmicroros.a" file.}
\end{figure}
...
The end result for these implementation are visible in the appendix \ref{sec:org9924867},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:org36a224d}
...
is showed and detailed in the figure \ref{fig:org7c0cd44} below.

The next figure \ref{fig:org79cb1c6} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:org7c0cd44}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:org79cb1c6}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org174a29b}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:org7c0cd44}, here are the main steps
...
is visible in the section \ref{sec:org13dda84}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
As for the previous section, the figure \ref{fig:org09c13a5} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org09c13a5}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:orgbaf6ab4}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:org8cc8be8}
...
in the figure \ref{fig:org916bb88} below, after the commands:
...
\caption{\label{fig:org916bb88}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsubsection{Running a ROS2 container}
\label{sec:org6a27dca}
...
and as presented in the section \ref{sec:org8cc8be8} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
on the schematic of the figure \ref{fig:orgc37586f} below.
...
\caption{\label{fig:orgc37586f}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{Building a eProsima bare-metal example agent}
\label{sec:org563cc21}
...
\label{sec:org9924867}

\subsection{main}
\label{sec:org2f53cd8}
...
\label{sec:org212e354}

\subsection{main}
\label{sec:orga589d07}
