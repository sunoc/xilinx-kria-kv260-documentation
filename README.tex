...
RTOS. The figure \ref{fig:org552bb9e} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/map.png}
\caption{\label{fig:org552bb9e}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}

\section{Boot firmware}
\label{sec:org3117469}
...
figure \ref{fig:orga284f71} below.
...
the IPv4 as manual, as visible in the figure \ref{fig:orga2130fc} hereafter.
...
shall now see the interface, as visible on the figure \ref{fig:org8ee4a20} below. If
...
\caption{\label{fig:orga284f71}Port to be used for the Ethernet Boot Recovery Tool access on the KR260 board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/ipv4set.png}
\caption{\label{fig:orga2130fc}IPv4 settings for accessing the Recovery Tool.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{img/recovery.png}
\caption{\label{fig:org8ee4a20}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsubsection{Updating the boot firmware}
\label{sec:orgcf1439d}
...
(as of 2023-08-30).}. The figure \ref{fig:org01d545f} below shows
...
\caption{\label{fig:org01d545f}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:org1368e5e}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:org65744d7} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:org65744d7}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\subsubsection{Enabling \texttt{remoteproc} with Device-Tree Overlay patching}
\label{sec:orgcb31546}
...
but also in this report appendix \ref{sec:orgbef9eb4}.
...
in the figure \ref{fig:orgb645d31} below, after the commands:
...
\caption{\label{fig:orgb645d31}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsubsection{Adding a swap partition}
\label{sec:org3f8ddfa}
...
The figure \ref{fig:orgabb7206} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:orgabb7206}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org94031fa}
...
\caption{\label{fig:orgd6b4410}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:org78683d3}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:org4a75236}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:org7a7650d}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:orga047f1c}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:orgd4d5cef}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org2b264f9}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org6869554} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org6869554}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}


Once your example project is built and you have a \texttt{.elf} file available, you can
jump directly to the \ref{sec:org9724bbf} section to see how to deploy and use your firmware.

The section in between will present setup specifically needed for micro-ROS.

\subsection{Enabling the Stream Buffer system}
\label{sec:org94456f4}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:org911485b}
and \ref{sec:org12ab0c4} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:org84a926a} and \ref{fig:org2f83d59} below will show you a setup that worked to have the IDE
...
\caption{\label{fig:org84a926a}Firstly, in the "C/C++ Build" settings of your firmware project, under the "Settings" menu, you should find the gcc compiler "Directories". In here you should add the "include" directory of your library. Be careful however, if your include files are in a second layer of directory (as it is the case for libmicroros) you will need to include each sub-directory individually, as visible in this figure.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{./img/vitis_new/include2.png}
\caption{\label{fig:org2f83d59}Secondly, in the gcc linker "Libraries", you can add the top level directory of your library. In our case, it is the directory that contains both the "include" directory added earlier, and also the "libmicroros.a" file.}
\end{figure}
...
The end result for these implementation are visible in the appendix \ref{sec:org0f4ffc8},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:orgfd208a8}
...
is showed and detailed in the figure \ref{fig:org2839d06} below.

The next figure \ref{fig:org9364447} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:org2839d06}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:org9364447}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org94031fa}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:org2839d06}, here are the main steps
...
is visible in the section \ref{sec:orgd6976b4}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
As for the previous section, the figure \ref{fig:org5bcb43f} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org5bcb43f}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:orgcf3635d}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:orge24af3f}
...
as presented in section \ref{sec:orga6939ac}.
...
and as presented in the section \ref{sec:orge24af3f} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
source /opt/ros/$ROS_DISTRO/setup.bash
...
on the schematic of the figure \ref{fig:org27b98be} below.
...
\caption{\label{fig:org27b98be}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{[irrelevent] Building a eProsima bare-metal example agent}
\label{sec:org896de73}
...
\label{sec:org0f4ffc8}

\subsection{main}
\label{sec:orgfbb3d30}
...
\label{sec:orgc5c8bf7}

\subsection{main}
\label{sec:orgc94295c}
...
\label{sec:orge1c92b4}
This part is an alternative to the previous section \ref{sec:orgeb21e5a},
...
board's latest updated boot firmware, as presented in the Section \ref{sec:org3117469}
...
will also be using this version.}\footnotetext[38]{\label{orgf446df9}\url{https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria+SOMs+Starter+Kits\#K26-Boot-Firmware-Updates}}.
\end{itemize}
...
setting the the DTG Settings, as visible in the Figure \ref{fig:org47538f8} below:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
\caption{\label{fig:org47538f8}In the hardware config, it is possible to select the openamp device tree source.}
\end{figure}
...
\caption{\label{fig:org2162d17}In the rootfs config, it is possible to select the openamp package.}
\end{figure}
...
\caption{\label{fig:org6a9b665}OpenSSH can be configure in the same rootfs config.}
\end{figure}
...
The figure \ref{fig:orge675b53} below shows a comparison for the data
...
\caption{\label{fig:orge675b53}A graph of the data transfer rate for individual packages to be transmitted and received back from the Linux.}
\end{figure}
