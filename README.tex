...
RTOS (Cortex R5F). The figure \ref{fig:orgdd20c68} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/map.png}
\caption{\label{fig:orgdd20c68}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}
...
figure \ref{fig:org14227d7} below.
...
the IPv4 as manual, as visible in the figure \ref{fig:org17b7da1} hereafter.
...
shall now see the interface, as visible on the figure \ref{fig:org9d1c631} below. If
...
\caption{\label{fig:org14227d7}Port to be used for the Ethernet Boot Recovery Tool access on the KR260 board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/ipv4set.png}
\caption{\label{fig:org17b7da1}IPv4 settings for accessing the Recovery Tool.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{img/recovery.png}
\caption{\label{fig:org9d1c631}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsection{Updating the boot firmware}
\label{sec:org7af3759}
...
(as of 2023-08-30).}. The figure \ref{fig:orge9549ec} below shows
...
\caption{\label{fig:orge9549ec}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:orgd843073}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:org328d2b1} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:org328d2b1}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\subsubsection{Enabling \texttt{remoteproc} with Device-Tree Overlay patching}
\label{sec:orgb772dd4}
...
but also in this report appendix \ref{sec:orgf306f6a}.
...
in the figure \ref{fig:orga2ab824} below, after the commands:
...
\caption{\label{fig:orga2ab824}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsubsection{Adding a swap partition}
\label{sec:org335dbe8}
...
The figure \ref{fig:orgf2c9b5f} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:orgf2c9b5f}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org6b67651}
...
\caption{\label{fig:org62e58e8}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:org95b857e}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:org07c7051}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:org7f4ea7f}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:org37ed7b8}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org02fc01a}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org1861599}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org900b095} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org900b095}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}
...
the figures \ref{fig:orgd7dcd16} and \ref{fig:orgf784a12} below will show you a setup that worked to have the IDE
...
\caption{\label{fig:orgd7dcd16}Firstly, in the "C/C++ Build" settings of your firmware project, under the "Settings" menu, you should find the gcc compiler "Directories". In here you should add the "include" directory of your library. Be careful however, if your include files are in a second layer of directory (as it is the case for libmicroros) you will need to include each sub-directory individually, as visible in this figure.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{./img/vitis_new/include2.png}
\caption{\label{fig:orgf784a12}Secondly, in the gcc linker "Libraries", you can add the top level directory of your library. In our case, it is the directory that contains both the "include" directory added earlier, and also the "libmicroros.a" file.}
\end{figure}
...
As for the previous section, the figure \ref{fig:org1fa6145} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:orgbb6fe7e}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:org726ddc8}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:org76f8243}
...
as presented in section \ref{sec:orge1ea128}.
...
and as presented in the section \ref{sec:org76f8243} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
source /opt/ros/$ROS_DISTRO/setup.bash
...
The end result for these implementation are visible in the appendix \ref{sec:org93325f7},
and can be reused as-if for the Kria board setups.

\subsubsection{[DEPRECATED] Memory allocators}
\label{sec:orgcef7c10}
...
is showed and detailed in the figure \ref{fig:org06baccd} below.

The next figure \ref{fig:orgfe899b9} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:org06baccd}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:orgfe899b9}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org6b67651}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:org06baccd}, here are the main steps
...
is visible in the section \ref{sec:org04d0729}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
\caption{\label{fig:org6623966}With a project meant to be build and run on the Cortex R5F, you must first produce the \texttt{.elf} binary file.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/debug_config.png}
\caption{\label{fig:org99f11a1}Then you can open the Debug Configuration window.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/debug_config2.png}
\caption{\label{fig:org88189db}All parameters should stay the same. You can then run "Debug".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/console.png}
\caption{\label{fig:orgfd558ac}The XSCT console will confirm the successful load of the firmware. If not, you will get error messages and should reboot the board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/run.png}
\caption{\label{fig:org1fa6145}Selecting the Cortex R5, core \#0, you can then run the application.}
\end{figure}

\clearpage
\section{micro-ROS XRCE-DDS Agent}
\label{sec:org95bdc10}
...
on the schematic of the figure \ref{fig:org95a2ca6} below.
...
\caption{\label{fig:org95a2ca6}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{Building a XRCE-DDS agent in a Docker}
\label{sec:org99e5f2c}
...
\begin{itemize}
\item The micro-ROS firmware is running on the R5F core, as presented in the section \ref{sec:orgd110cdd}.
\item On the same Linux you plan to run the node, you will need to run and keep the Agent, as presented
in the section \ref{sec:org95bdc10}.
...
\label{sec:org93325f7}

\subsection{main}
\label{sec:org931ec05}
...
\label{sec:orgdd3beb7}

\subsection{main}
\label{sec:org0c0d074}
...
\label{sec:orgdea20ba}
This part is an alternative to the previous section \ref{sec:org7d16a3c},
...
will also be using this version.}\footnotetext[39]{\label{org7048f3b}\url{https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria+SOMs+Starter+Kits\#K26-Boot-Firmware-Updates}}.
\end{itemize}
...
setting the the DTG Settings, as visible in the Figure \ref{fig:org295c2d4} below:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
\caption{\label{fig:org295c2d4}In the hardware config, it is possible to select the openamp device tree source.}
\end{figure}
...
\caption{\label{fig:orga2d071d}In the rootfs config, it is possible to select the openamp package.}
\end{figure}
...
\caption{\label{fig:org056b4f6}OpenSSH can be configure in the same rootfs config.}
\end{figure}
...
The figure \ref{fig:org9a9f2a5} below shows a comparison for the data
...
\caption{\label{fig:org9a9f2a5}A graph of the data transfer rate for individual packages to be transmitted and received back from the Linux.}
\end{figure}
