...
RTOS. The figure \ref{fig:orga45a38f} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/map.png}
\caption{\label{fig:orga45a38f}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}

\section{Boot firmware update}
\label{sec:org0301754}
...
figure \ref{fig:org504c931} below.
...
the IPv4 as manual, as visible in the figure \ref{fig:orgb428156} hereafter.
...
shall now see the interface, as visible on the figure \ref{fig:orgd8d04de} below. If
...
\caption{\label{fig:org504c931}Port to be used for the Ethernet Boot Recovery Tool access on the KR260 board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/ipv4set.png}
\caption{\label{fig:orgb428156}IPv4 settings for accessing the Recovery Tool.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{img/recovery.png}
\caption{\label{fig:orgd8d04de}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsubsection{Updating the boot firmware}
\label{sec:org0d1ac46}
...
(as of 2023-08-30).}. The figure \ref{fig:org523817b} below shows
...
\caption{\label{fig:org523817b}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsubsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:org83dbaea}
...
\subsubsection{Network and admin setups}
\label{sec:orgd6f14aa}
...
\item \texttt{root} password
\label{sec:org94bb7fd}
...
\item Adding Xilinx specific repositories
\label{sec:org345aeda}
...
\item Purging \texttt{snap}
\label{sec:org88905cb}
...
\item Other unused heavy packages
\label{sec:org0551509}
...
\item Slow boot services to disable
\label{sec:org279836e}
...
\item Jupyter notebook setup
\label{sec:org8d08578}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:orgc901f81} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:orgc901f81}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\item Enabling \texttt{remoteproc} with Device-Tree Overlay patching
\label{sec:org225be21}
...
but also in this report appendix \ref{sec:org0088056}.
...
in the figure \ref{fig:org8547f9a} below, after the commands:
...
\caption{\label{fig:org8547f9a}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\item Adding a swap partition
\label{sec:org0df246a}
...
\item {\bfseries\sffamily TODO} Using a PetaLinux kernel in Ubuntu
\label{sec:orgb7bba9a}


\pagebreak
\appendix
\end{enumerate}
\subsection{DTO patch}
\label{sec:orgeacc1f2}
...
\label{sec:orgb89e9de}
\subsection{Initial setup}
\label{sec:orga21c040}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:org3141403}
and \ref{sec:org9e8e8b5} the end of this report.\}} will be downloaded from my repository;
...
The figure \ref{fig:orgbc11413} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:orgbc11413}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org2785950}
...
\caption{\label{fig:org32ceb86}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:org4b046c2}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:orgd7faae3}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:orgd969ec1}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:org8adf681}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org8d544fb}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org48ace2e}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org44c73d9} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org44c73d9}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}
...
the figures \ref{fig:org39007dd} and \ref{fig:orgbeb0450} below will show you a setup that worked to have the IDE
...
\caption{\label{fig:org39007dd}Firstly, in the "C/C++ Build" settings of your firmware project, under the "Settings" menu, you should find the gcc compiler "Directories". In here you should add the "include" directory of your library. Be careful however, if your include files are in a second layer of directory (as it is the case for libmicroros) you will need to include each sub-directory individually, as visible in this figure.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{./img/vitis_new/include2.png}
\caption{\label{fig:orgbeb0450}Secondly, in the gcc linker "Libraries", you can add the top level directory of your library. In our case, it is the directory that contains both the "include" directory added earlier, and also the "libmicroros.a" file.}
\end{figure}
...
\label{sec:org811b25b}
\subsection{RPMsg Cortex R5F demonstration firmware}
\label{sec:org04e0fa9}
\subsection{RPMsg \texttt{echo\_test} software}
\label{sec:orgef7e060}
...
The end result for these implementation are visible in the appendix \ref{sec:org48b888a},
and can be reused as-if for the Kria board setups.

\subsection{[DEPRECRTED] Memory allocators}
\label{sec:orgcf29fcb}
...
is showed and detailed in the figure \ref{fig:org3f8f96b} below.

The next figure \ref{fig:orga0a8bd5} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:org3f8f96b}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:orga0a8bd5}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org2785950}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:org3f8f96b}, here are the main steps
...
is visible in the section \ref{sec:orgef7e060}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
\caption{\label{fig:org772c66f}With a project meant to be build and run on the Cortex R5F, you must first produce the \texttt{.elf} binary file.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/debug_config.png}
\caption{\label{fig:org5f12224}Then you can open the Debuc Configuration window.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/debug_config2.png}
\caption{\label{fig:orgfa1a1ea}All parameters should stay the same. You can then run "Debug".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/console.png}
\caption{\label{fig:org9986d5e}The XSCT console will confirm the successful load of the firmware. If not, you will get error messages and should reboot the board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/run.png}
\caption{\label{fig:orge5745b0}Selecting the Cortex R5, core \#0, you can then run the application.}
\end{figure}

\pagebreak
\section{Running a ROS2 node}
\label{sec:org94c6151}
...
As for the previous section, the figure \ref{fig:org16c75c3} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org16c75c3}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:org191fb7b}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:orgb8c95fa}
...
as presented in section \ref{sec:org83a3b97}.
...
and as presented in the section \ref{sec:orgb8c95fa} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
source /opt/ros/$ROS_DISTRO/setup.bash
...
on the schematic of the figure \ref{fig:orgc23d3e2} below.
...
\caption{\label{fig:orgc23d3e2}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{Building a XRCE-DDS agent in a Docker}
\label{sec:orgb326570}
...
\label{sec:org48b888a}

\subsection{main}
\label{sec:org96d1964}
...
\label{sec:orgacb5554}

\subsection{main}
\label{sec:orgfafe0eb}
...
\label{sec:orgfee612c}
This part is an alternative to the previous section \ref{sec:org91ad08d},
...
will also be using this version.}\footnotetext[40]{\label{org2903f24}\url{https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria+SOMs+Starter+Kits\#K26-Boot-Firmware-Updates}}.
\end{itemize}
...
setting the the DTG Settings, as visible in the Figure \ref{fig:orgde1913c} below:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
\caption{\label{fig:orgde1913c}In the hardware config, it is possible to select the openamp device tree source.}
\end{figure}
...
\caption{\label{fig:org1ed3535}In the rootfs config, it is possible to select the openamp package.}
\end{figure}
...
\caption{\label{fig:org308c9b4}OpenSSH can be configure in the same rootfs config.}
\end{figure}
...
The figure \ref{fig:org93eb32a} below shows a comparison for the data
...
\caption{\label{fig:org93eb32a}A graph of the data transfer rate for individual packages to be transmitted and received back from the Linux.}
\end{figure}
