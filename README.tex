...
RTOS. The figure \ref{fig:org0ab384e} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/map.png}
\caption{\label{fig:org0ab384e}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}

\section{Boot firmware}
\label{sec:orgd327f51}
...
figure \ref{fig:orgac7b0d8} below.
...
the IPv4 as manual, as visible in the figure \ref{fig:org95691f3} hereafter.
...
shall now see the interface, as visible on the figure \ref{fig:org9287bea} below. If
...
\caption{\label{fig:orgac7b0d8}Port to be used for the Ethernet Boot Recovery Tool access on the KR260 board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/ipv4set.png}
\caption{\label{fig:org95691f3}IPv4 settings for accessing the Recovery Tool.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{img/recovery.png}
\caption{\label{fig:org9287bea}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsubsection{Updating the boot firmware}
\label{sec:orgebeaac5}
...
(as of 2023-08-30).}. The figure \ref{fig:orgb895457} below shows
...
\caption{\label{fig:orgb895457}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:orgd30e2be}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:orgf8cd562} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:orgf8cd562}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\subsubsection{Enabling \texttt{remoteproc} with Device-Tree Overlay patching}
\label{sec:org2f15852}
...
but also in this report appendix \ref{sec:org16144f4}.
...
in the figure \ref{fig:org624f3ba} below, after the commands:
...
\caption{\label{fig:org624f3ba}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsubsection{Adding a swap partition}
\label{sec:org0a9d7e9}
...
\label{sec:org5580d0e}
\section{Setting up a Vitis IDE project}
\label{sec:org3911bd5}
...
The figure \ref{fig:org8449f0a} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:org8449f0a}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsubsection{Setting up the IDE}
\label{sec:org61f859a}
...
\item Platform configuration file
\label{sec:orgf8b2576}
...
\item\relax [DEPRECATED] Platform configuration file generation
\label{sec:org35c6908}
...
\subsubsection{Setting up and building a new project for the Kria board}
\label{sec:org61c290f}
...
\caption{\label{fig:org4c48f27}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:orgdfef52a}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:orgc0ceb03}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:orgd65de4b}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:org356b61d}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org219712e}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org7d186a1}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:orgaabdb76} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:orgaabdb76}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}
...
\subsubsection{Enabling the Stream Buffer system}
\label{sec:orgfb9487a}
...
\subsubsection{Importing an existing project}
\label{sec:orgd146cb4}
\section{{\bfseries\sffamily DONE} R5F firmware without an IDE}
\label{sec:org5b78bc1}
\begin{itemize}
\item State "DONE"       from "TODO"       \textit{[2024-02-13 Tue 15:28]}
\end{itemize}
The Section \ref{sec:org3911bd5} was merely a presentation on how
bootstrap a brand new development project using Xilinx provided templates in their crap IDE.

Once the project exist, one can simply compile it again using the generated \texttt{Makefile}.

The information on how to handle the project are available directly in the
repository of said project\footnote{\url{https://gitlab.com/sunoc/libmicroros\_kv260}} and it's various branches for ongoing feature
development.


\pagebreak
\section{RPMsg \texttt{echo\_test} software}
\label{sec:orgc422664}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:org16e9868}
and \ref{sec:orgc63fd96} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:orgee99249} and \ref{fig:org65f7e42} below will show you a setup that worked to have the IDE
...
The end result for these implementation are visible in the appendix \ref{sec:org15c50ca},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:orgcdc0bb7}
...
is showed and detailed in the figure \ref{fig:orgbb19114} below.

The next figure \ref{fig:org143989b} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:orgbb19114}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:org143989b}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org61f859a}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:orgbb19114}, here are the main steps
...
is visible in the section \ref{sec:orgc422664}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
\pagebreak
\section{Loading the firmware using JTAG and Vitis IDE}
\label{sec:org6d30585}
This is more of an alternative way, but it is possible to load the Client
firmware onto the Cortex R5F onto the boad from a host PC IDE, without booting or
...
There are two key advantaged to using this methode:
\begin{itemize}
...
In my case, I'll have to go to \texttt{\textasciitilde{}/tools/Xilinx/Vitis/2023.2}, but your milage might vary
depending on the settings you choose during the IDE installation and also the version of
the IDE itself.

\subsubsection{Drivers}
\label{sec:org953e590}
...
\subsubsection{\texttt{dialout} access}
\label{sec:org73617a3}
...
\subsubsection{TCL JTAG boot}
\label{sec:orgb1699fc}
A short TCL program was provided by Honda-sensei and should be loaded in order
for the board to boot in JTAG mode.
The following code block shows the TCL, and it should be saved somewhere, for exemple
...
\subsection{Vitis IDE classic}
\label{sec:orgdb919fd}
...
\caption{\label{fig:org2f38526}With a project meant to be build and run on the Cortex R5F, you must first produce the \texttt{.elf} binary file.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/debug_config.png}
\caption{\label{fig:orgae14503}Then you can open the Debuc Configuration window.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/debug_config2.png}
\caption{\label{fig:org3dcb903}All parameters should stay the same. You can then run "Debug".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/console.png}
\caption{\label{fig:orga40ec44}The XSCT console will confirm the successful load of the firmware. If not, you will get error messages and should reboot the board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jtag/run.png}
\caption{\label{fig:org927a840}Selecting the Cortex R5, core \#0, you can then run the application.}
\end{figure}

\pagebreak
\section{Running a ROS2 node}
\label{sec:org711ed9b}
...
As for the previous section, the figure \ref{fig:org0f5e6ec} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org0f5e6ec}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:org0ff500a}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:org2dd2c1f}
...
as presented in section \ref{sec:org931faff}.
...
and as presented in the section \ref{sec:org2dd2c1f} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
source /opt/ros/$ROS_DISTRO/setup.bash
...
on the schematic of the figure \ref{fig:orgd66501e} below.
...
\caption{\label{fig:orgd66501e}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{Building a XRCE-DDS agent in a Docker}
\label{sec:org25422a3}
...
\label{sec:org15c50ca}

\subsection{main}
\label{sec:org74fcc2f}
...
\label{sec:org6ccad5e}

\subsection{main}
\label{sec:orgf352719}
...
\label{sec:orgd20e016}
This part is an alternative to the previous section \ref{sec:org69bdc58},
...
board's latest updated boot firmware, as presented in the Section \ref{sec:orgd327f51}
...
will also be using this version.}\footnotetext[42]{\label{orgafd64cd}\url{https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria+SOMs+Starter+Kits\#K26-Boot-Firmware-Updates}}.
\end{itemize}
...
Testing, if building on Debian 12 the following variante of the same command has been confirmed to work:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo apt-get install iproute2 gawk python3 build-essential gcc git make \
...
setting the the DTG Settings, as visible in the Figure \ref{fig:org20ab156} below:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
\caption{\label{fig:org20ab156}In the hardware config, it is possible to select the openamp device tree source.}
\end{figure}
...
\caption{\label{fig:org8ec9898}In the rootfs config, it is possible to select the openamp package.}
\end{figure}
...
\caption{\label{fig:orgefa45c7}OpenSSH can be configure in the same rootfs config.}
\end{figure}
...
The figure \ref{fig:org8ae5eaf} below shows a comparison for the data
...
\caption{\label{fig:org8ae5eaf}A graph of the data transfer rate for individual packages to be transmitted and received back from the Linux.}
\end{figure}
