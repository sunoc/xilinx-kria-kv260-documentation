...
RTOS. The figure \ref{fig:orge7e6234} below shows a schematic view of the overall system we
are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map.png}
\caption{\label{fig:orge7e6234}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}

\section{Boot firmware}
\label{sec:orgc9bddf8}
...
figure \ref{fig:org49851cd} below.
...
the IPv4 as manual, as visible in the figure \ref{fig:org9f7013a} hereafter.
...
shall now see the interface, as visible on the figure \ref{fig:orgeccb70f} below. If
...
\caption{\label{fig:org49851cd}Port to be used for the Ethernet Boot Recovery Tool access on the KR260 board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/ipv4set.png}
\caption{\label{fig:org9f7013a}IPv4 settings for accessing the Recovery Tool.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{img/recovery.png}
\caption{\label{fig:orgeccb70f}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsubsection{Updating the boot firmware}
\label{sec:orgaf497f4}
...
(as of 2023-08-30).}. The figure \ref{fig:org62b4e93} below shows
...
\caption{\label{fig:org62b4e93}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:org59c610e}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:org24b0cc6} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/jupyter.png}
\caption{\label{fig:org24b0cc6}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\section{Installing Linux (PetaLinux option)\hfill{}\textsc{WORK\_IN\_PROGRESS}}
\label{sec:org454a066}
...
\caption{\label{fig:orgf86df9c}Hey, listen \textasciitilde{}}
\end{figure}
...
\caption{\label{fig:orge0b4b63}Hey, listen !!}
\end{figure}


The more general option allows to enable other stuff related to OpenAMP:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
petalinux-config
\end{minted}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/yocto_openamp2.png}
\caption{\label{fig:org97e9f7c}Hey, listen !}
\end{figure}


It is then possible to build and package the image. Most critically compared to the example,
the \texttt{openamp.dtb} overlay must be used in order for OpenAMP and subsequently RPMsg to become usable.
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
petalinux-build
#petalinux-package --wic --bootfiles "ramdisk.cpio.gz.u-boot boot.scr Image ../../pre-built/linux/images/openamp.dtb"
...
controll system, as visible in the \texttt{dmesg} log.
...
but also in this report appendix \ref{sec:orgc1a98f1}.
...
The figure \ref{fig:orgb5ba7a9} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:orgb5ba7a9}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org15cd0d6}
...
\caption{\label{fig:org179796d}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:org0c61f8d}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:orgcf54193}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:org89412a5}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:org3c72ebd}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:orgaf15c91}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org7587a35}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org0681545} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org0681545}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}


Once your example project is built and you have a \texttt{.elf} file available, you can
jump directly to the \ref{sec:orgac845db} section to see how to deploy and use your firmware.

The section in between will present setup specifically needed for micro-ROS.

\subsection{Enabling the Stream Buffer system}
\label{sec:org955909e}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:org26d3df1}
and \ref{sec:orgbd1c482} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:org659103b} and \ref{fig:org7cc05bb} below will show you a setup that worked to have the IDE
...
\caption{\label{fig:org659103b}Firstly, in the "C/C++ Build" settings of your firmware project, under the "Settings" menu, you should find the gcc compiler "Directories". In here you should add the "include" directory of your library. Be careful however, if your include files are in a second layer of directory (as it is the case for libmicroros) you will need to include each sub-directory individually, as visible in this figure.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{./img/vitis_new/include2.png}
\caption{\label{fig:org7cc05bb}Secondly, in the gcc linker "Libraries", you can add the top level directory of your library. In our case, it is the directory that contains both the "include" directory added earlier, and also the "libmicroros.a" file.}
\end{figure}
...
The end result for these implementation are visible in the appendix \ref{sec:org65dbacb},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:org8bb6f41}
...
is showed and detailed in the figure \ref{fig:orgf843dfb} below.

The next figure \ref{fig:org0d72537} show a more visual representation of the two tasks and the
functions used in them.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth]{./img/tasks.png}
\caption{\label{fig:orgf843dfb}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
\caption{\label{fig:org0d72537}Functions architecture for the Client firmware.}
\end{figure}
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org15cd0d6}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:orgf843dfb}, here are the main steps
...
is visible in the section \ref{sec:org0e85d9b}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
As for the previous section, the figure \ref{fig:org300807e} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org300807e}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.55\textwidth]{./img/agent_arch.png}
\caption{\label{fig:org174fc42}Methods architecture for the modified Agent node.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:orgca92820}
...
in the figure \ref{fig:orgf26d23e} below, after the commands:
...
\caption{\label{fig:orgf26d23e}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsubsection{Running a ROS2 container}
\label{sec:org3371657}
...
and as presented in the section \ref{sec:orgca92820} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
on the schematic of the figure \ref{fig:orgee6e6eb} below.
...
\caption{\label{fig:orgee6e6eb}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}

\subsection{Building a eProsima bare-metal example agent}
\label{sec:orgf51e786}
...
\label{sec:org65dbacb}

\subsection{main}
\label{sec:org71ba7ae}
...
\label{sec:org2aadd33}

\subsection{main}
\label{sec:orgd815d72}
