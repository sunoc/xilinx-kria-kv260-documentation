...
capable to run a RTOS. The figure \ref{fig:org1b19930} below shows a schematic
view of the overall system we are trying to archive.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map.png}
\caption{\label{fig:org1b19930}The Linux and ROS2 environment (orange) will communicate with the real-time, FreeRTOS and micro-ROS side (blue) using RPMsg (shared memory).}
\end{figure}


\section{Boot firmware}
\label{sec:org66fb884}
...
the figure \ref{fig:org96a695a} below. If the page struggle to appear,
\end{itemize}
...
\caption{\label{fig:org96a695a}The recovery tool for the board, access from Firefox. We can see board information at the center, and the tools to upload the firmware at the bottom of the page.}
\end{figure}

\subsection{Updating the boot firmware}
\label{sec:org9e40cf4}
...
(as of 2023-08-30).}. The figure \ref{fig:orgc0d636b} below shows
...
\caption{\label{fig:orgc0d636b}The Linux operating system (red border)runs on the APU (application, general purpose) side of the Kria board CPU. It is the base layer for the ROS2 system.}
\end{figure}

\subsection{Preparing and booting a Ubuntu 22.04 media}
\label{sec:orgb6ffce0}
...
but also in this report appendix \ref{sec:org60f2740}.
...
The figure \ref{fig:org729fab3} below shows where the real-time firmware
is positioned in the global project.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_microros.png}
\caption{\label{fig:org729fab3}The FreeRTOS firmware and it's application (red border) are running on the real-time capable side of the Kria CPU. A micro-ROS application is shown here, but any real-time firmware will be deployed in the same way.}
\end{figure}

\subsection{Setting up the IDE}
\label{sec:org7a2b1c5}
...
\caption{\label{fig:org7012285}We are starting with creating a "New Application Project" You should be greeted with this wizard window. Next.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.5\textwidth]{./img/vitis_new/project2.png}
\caption{\label{fig:orgf5a8e4d}For the platform, we need to get our build Kria configuration. In the "Create a new platform" tab, click the "Browse\ldots{}" button.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project3.png}
\caption{\label{fig:org0a353fa}In the file explorer, we should navigate in the "k26" directory, where the configuration file was build. From here we are looking for a ".xsa" file, located in a "hw" directory, as visible.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project4.png}
\caption{\label{fig:orgbbee7a2}With the configuration file loaded, we can now select a name for our platform, but most importantly, we have to select the "psu Cortex5 0" core as a target. The other, Cortex 53 is the APU running Linux.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project5.png}
\caption{\label{fig:orgd2beb5e}In this next window, we can give a name to our firmware project. It is also critical here to select the core we want to build for. Once again, we want to use the "psu cortex5 0".}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project6.png}
\caption{\label{fig:orgd837018}Here, we want to select "freertos10 xilinx" as our Operating System. The rest can remain unchanged.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/vitis_new/project7.png}
\caption{\label{fig:org4c33819}Finally, we can select the demonstration template we are going to use; here we go with "OpenAMP echo-test" since we want to have some simple try of the RPMsg system. Finish.}
\end{figure}
...
The values in the figure \ref{fig:org8b1c941} below look different from what could be set in the DTO for the Linux side, but they appear to
...
\caption{\label{fig:org8b1c941}lscript.ld memory configuration for the firmware memory setup. The same file is available as a whole in this repository's src directory.}
\end{figure}


Once your example project is built and you have a \texttt{.elf} file available, you can
jump directly to the \ref{sec:org30b205f} section to see how to deploy and use your firmware.

The section in between will present setup specifically needed for micro-ROS.

\subsection{Enabling the Stream Buffer system}
\label{sec:org35b1ee7}
...
Both configuration files\footnote{Both files are also visible in the appendixes \ref{sec:orgda66bb7}
and \ref{sec:orgb9aa580} the end of this report.\}} will be downloaded from my repository;
...
the figures \ref{fig:org0112fde} and \ref{fig:orgcd1873b} below will show you a setup that worked to have the IDE
...
The end result for these implementation are visible in the appendix \ref{sec:org766dd46},
and can be reused as-if for the Kria board setups.

\subsection{Memory allocators}
\label{sec:org740d37d}
...
is showed and detailed in the figure \ref{fig:orgde3903c} below.
...
\caption{\label{fig:orgde3903c}Two tasks are being run concurrently in order to manage the communication situation, with binary semaphore-based lock-unlock system.\\[0pt]
...
Xilinx's IDE, which setup was presented in the section \ref{sec:org7a2b1c5}, however it was tested
...
Beyond the general two-tasks behavior of the figure \ref{fig:orgde3903c}, here are the main steps
...
is visible in the section \ref{sec:orgc58580a}.}:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo -s
...
As for the previous section, the figure \ref{fig:org6de01cd} below shows what part
of the overall system we are talking about here.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{./img/map_ros.png}
\caption{\label{fig:org6de01cd}The ROS2 middle (red border) runs on top of the Linux, on the general-purpose core of the Kria board.}
\end{figure}

\subsection{On the host Linux ("bare-metal")}
\label{sec:orgc29ebd2}
...
in the figure \ref{fig:org811b27f} below, after the commands:
...
\caption{\label{fig:org811b27f}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}


\subsubsection{Running a ROS2 container}
\label{sec:org9496cbd}
...
and as presented in the section \ref{sec:orgc29ebd2} above:
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
...
on the schematic of the figure \ref{fig:org20e6a5f} below.
...
\caption{\label{fig:org20e6a5f}The agent (red border) allows for a micro-ROS instance to communicate with a ROS2 system. It is deployed on the Linux side, as a ROS2 node.}
\end{figure}


\subsection{Building a eProsima bare-metal example agent}
\label{sec:org005182c}
...
\label{sec:org766dd46}

\subsection{main}
\label{sec:orgaaf6f02}
...
\label{sec:orgf40d498}

\subsection{main}
\label{sec:orga130b56}
