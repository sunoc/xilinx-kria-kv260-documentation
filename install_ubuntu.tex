...
\subsection{\texttt{root} password}
\label{sec:org4c553af}
...
in the figure \ref{fig:orgc8407af} below, after the commands:
...
\caption{\label{fig:orgc8407af}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsection{Purging \texttt{snap}}
\label{sec:org3998d79}
...
\subsection{Other unused heavy packages}
\label{sec:org0602b44}
...
\subsection{Slow boot services to disable}
\label{sec:orga666403}
...
\subsection{Adding a swap partition}
\label{sec:org5285167}
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:org032dfcc} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:org032dfcc}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\subsection{{\bfseries\sffamily TODO} Using a PetaLinux kernel in Ubuntu}
\label{sec:org86ae7ff}

\section{RPMsg standalone evaluation}
\label{sec:org2e7e5b1}
\subsection{{\bfseries\sffamily TODO} RPMsg Cortex R5F demonstration firmware}
\label{sec:org8be88fa}
\subsection{RPMsg \texttt{echo\_test} software}
\label{sec:orgcc5afb9}
