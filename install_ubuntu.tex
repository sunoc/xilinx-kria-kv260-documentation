...
in the figure \ref{fig:org6c7a8f1} below, after the commands:
...
\caption{\label{fig:org6c7a8f1}The return of a successful run of the \texttt{hello world} test Docker container.}
\end{figure}

\subsection{[recommended] Purging \texttt{snap}}
\label{sec:org463d65e}
...
\label{sec:org761ae3c}
\subsubsection{Kernel \texttt{cmd} edit}
\label{sec:orge230584}
\begin{minted}[frame=single,framesep=2mm,baselinestretch=1.2,linenos,breaklines,fontsize=\footnotesize]{sh}
sudo nano /etc/default/flash-kernel
...
Eventually creating Notebooks and stuff, it is possible to obtain a situation like shown in the figure \ref{fig:orgbf8b972} below.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{img/jupyter.png}
\caption{\label{fig:orgbf8b972}A test Jupyter Notebook for CSV data analysis.}
\end{figure}

\subsection{[TODO] Using a PetaLinux kernel in Ubuntu}
\label{sec:org187214a}
TBD

\section{RPMsg standalone evaluation}
\label{sec:orgdffdd8d}
\subsection{[TODO] RPMsg Cortex R5F demonstration firmware}
\label{sec:orgee92f91}
\subsection{RPMsg \texttt{echo\_test} software}
\label{sec:org2548360}
