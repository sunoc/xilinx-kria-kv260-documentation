...
\date{}
\author{Vincent Conus  -  Source availbe at \github{https://github.com/sunoc/xilinx-kria-kv260-documentation}}
\begin{document}
...
\begin{itemize}
\item A slideshow (JP) from Fixstar employees presents how to use the device tree to enable the communication between the cores. \\
  \url{https://speakerdeck.com/fixstars/fpga-seminar-12-fixstars-corporation-20220727}
...
\begin{tcolorbox}
\begin{verbatim}
wget https://raw.githubusercontent.com/sunoc/xilinx-kria-\
kv260-documentation/main/src/system.patch
...
\begin{verbatim}
wget https://raw.githubusercontent.com/sunoc/xilinx-kria-\
kv260-documentation/main/src/custom_r5f_toolchain.cmake

wget https://raw.githubusercontent.com/sunoc/xilinx-kria-\
kv260-documentation/main/src/custom_r5f_colcon.meta
...
\pagebreak
\subsection{WIP::: Static library warnings}
\label{sec:stat-libr-warn}
In this extra section, I want to take notes about the large amount of warning that appears
when building the micro-ROS static library.

\begin{itemize}
\item \verb|Package 'rcutils' exports library 'dl' couldn't be found|:\\
  A warning that appears for almost every package. 
  This warning is referenced in a \href{https://github.com/micro-ROS/freertos_apps/issues/37}{Issue ticket on micro-ROS GitHub page}. 
  Need to investigate more since the issue is marked as closed, and seemed to have been solved by user ncnynl.
\item \verb|warning: array subscript has type 'char' [-Wchar-subscripts]|:\\
  No idea what's going on here.
\item \verb|warning: implicit declaration of function 'usleep';|,\\
      \verb|warning: implicit declaration of function 'clock_gettime'|:\\
      Normal, the time-related library will be added in the Vitis project.
\item \verb|warning: implicit declaration of function 'strdup';|: Feels similar to the ``usleep'' warning, but need to be sure.
\item The package ``rcutils'' displays a lot of warnings. Need to check these in detail.
\end{itemize}
...
% -------------------------------------------------------------------------------
\section{WIP:: Using micro-ROS in a Kria demonstration project}
\label{sec:kriademomicroros}
Beyond the inclusion of the library, actually using the micro-ROS system
within an external project require more than just importing the needed
functions.

Indeed, if you would be just adding the various function for sending messages to the
general ROS2 network, you would face issues with three key aspects, namely:
\begin{itemize}
...
All three points requires some custom function, specific for the board.
...
The end result is visible in the appendix \ref{sec:timefunctions}.


\subsection{Custom memory allocator functions}
\label{sec:memallocators}

\subsection{Custom transport layer functions}
...
Having now the latest version of our firmware (with or without the included micro-ROS library) loaded onto our Kria's Linux, we want to load and run it
...
It is also important to note that the \verb|echo_test| part is specific for the RPMsg base
demonstration firmware. It is not to be used for other firmware (including the one involving micro-ROS).
\begin{tcolorbox}
\begin{verbatim}
sudo -s
